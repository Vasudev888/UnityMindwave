...
public class DiamondGridMoverUDP : MonoBehaviour
{
    ...
    //Section Enlarge and Minimize Panel
    ...
    private void ReceiveCallback(IAsyncResult ar)
    {
        try
        {
            lock (lockObject)
            {
                if (isClosing)
                {
                    return; // Exit if the application is closing
                }
                ...
                // Continue receiving data if not closing
                if (!isClosing)
                {
                    udpClient.BeginReceive(new AsyncCallback(ReceiveCallback), null);
                }
    ...
    {
        panelImage.transform.localScale = new Vector3(1, 1, 1);
        CloseButton.SetActive(true);
    }

    public void CloseEnlargedPanel()
    {
        panelImage.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
        CloseButton.SetActive(false);
    }

    public void ToggleHeatmapVisibility()
    {
        isHeatmapVisible = !isHeatmapVisible; // Toggle the visibility flag
        heatmapDisplay.enabled = isHeatmapVisible; // Enable or disable the display based on flag
    }
